% Workshop OLG Transition Path 2
% Only change from 1 is that the tax cut doesn't occur until period 10 (only change is ParamPath)
% The early setup is much the same as we have seen elsewhere.
% Lines 129-175ish are the initial (tau=0.1) and final (tau=0.05) stationary
% general eqm. We need an initial agent dist and a final value fn (and we
% probably want to use the final p_eqm when creating our guess for
% PricePath0).
% Then, line 178ish, we start on the transition path.
...
ParamPath.tau=[tau_initial*ones(1,9), tau_final*ones(1,T-9)]; % exogenous parameter path
% This is the reform that we want to model, in period 1 announce that tax
% will be cut to tau=0.05 in period 10 (path is periods 1 to T)
...
PricePath0.r=[p_eqm_init.r*ones(1,10),linspace(p_eqm_init.r, p_eqm_final.r,ceil(T/2)), p_eqm_final.r*ones(1,T-ceil(T/2)-10)];
PricePath0.w=[p_eqm_init.w*ones(1,10),linspace(p_eqm_init.w, p_eqm_final.w,ceil(T/2)), p_eqm_final.w*ones(1,T-ceil(T/2)-10)];
