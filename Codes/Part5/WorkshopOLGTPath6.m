% Workshop OLG Transition Path 6

% Two permanent types, 'scaredycat' and 'curiouscat' which differ in sigma (risk aversion)
% Reform is same as in Example 1
...
% Risk aversion differs by permanent type
Params.sigma.curiouscat=1.5; % curvature of consumption in utility fn
Params.sigma.scaredycat=3; % curvature of consumption in utility fn
...
% Mass of agents of each type
PTypeDistParamNames={'ptypemass'};
Params.ptypemass=[0.7,0.3];
...
[p_eqm_init,~,GEcondns_init]=HeteroAgentStationaryEqm_Case1_FHorz_PType(n_d, n_a, n_z, N_j,Names_i, [], pi_z, d_grid, a_grid, z_grid, jequaloneDist, ReturnFn, FnsToEvaluate, GeneralEqmEqns, Params, DiscountFactorParamNames, AgeWeightParamNames, PTypeDistParamNames, GEPriceParamNames,heteroagentoptions, simoptions, vfoptions);
...
StationaryDist_init=StationaryDist_Case1_FHorz_PType(jequaloneDist,AgeWeightParamNames,PTypeDistParamNames,Policy_init,n_d,n_a,n_z,N_j,Names_i,pi_z,Params,simoptions);
...
[p_eqm_final,~,GEcondns_final]=HeteroAgentStationaryEqm_Case1_FHorz_PType(n_d, n_a, n_z, N_j,Names_i, [], pi_z, d_grid, a_grid, z_grid, jequaloneDist, ReturnFn, FnsToEvaluate, GeneralEqmEqns, Params, DiscountFactorParamNames, AgeWeightParamNames, PTypeDistParamNames, GEPriceParamNames,heteroagentoptions, simoptions, vfoptions);
% Done, the general eqm prices are in p_eqm
% GEcondns tells us the values of the GeneralEqmEqns, should be near zero
...
StationaryDist_final=StationaryDist_Case1_FHorz_PType(jequaloneDist,AgeWeightParamNames,PTypeDistParamNames,Policy_init,n_d,n_a,n_z,N_j,Names_i,pi_z,Params,simoptions);
% Calculate various stats
...
% And go!
PricePath=TransitionPath_Case1_FHorz_PType(PricePath0, ParamPath, T, V_final, AgentDist_init, jequaloneDist, n_d, n_a, n_z, N_j, Names_i,d_grid,a_grid,z_grid, pi_z, ReturnFn, FnsToEvaluate, GeneralEqmEqns, Params, DiscountFactorParamNames, AgeWeightParamNames, PTypeDistParamNames ,transpathoptions, simoptions, vfoptions);
...
subplot(2,1,1); plot(1:1:T,AggVarsPath.K.Mean,'b')
hold on
subplot(2,1,1); plot(1:1:T,AggVarsPath.K.scaredycat.Mean,'b--')
subplot(2,1,1); plot(1:1:T,AggVarsPath.K.curiouscat.Mean,'b.')
plot(-3:1:0,AllStats_init.K.Mean*ones(1,4),'r')
plot(-3:1:0,AllStats_init.K.scaredycat.Mean*ones(1,4),'r--')
plot(-3:1:0,AllStats_init.K.curiouscat.Mean*ones(1,4),'r.')
hold off
legend('Agg','scaredy cat','curious cat')
xlim([-3,T])
