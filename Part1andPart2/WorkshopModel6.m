% Workshop Model 6
% Permanent types by number, Names_i.
...
Names_i={'patient','impatient'}; % names of the permanent types
...
Params.beta.patient=0.99;
Params.beta.impatient=0.9;
% Use the names of the permanent types to set different values for them

% Tell VFI Toolkit the name of the parameter which is the masses of the permanent types
PTypeDistParamNames={'massofptypes'};
% Set the masses
Params.massofptypes=[0.5,0.5]; % Note: ordering follows that in Names_i
...
FnsToEvaluate.beta=@(h,aprime,a,z,beta) beta; % discount factor
...
beta_i=[AllStats.beta.patient.Mean,AllStats.beta.impatient.Mean]
% Since this stat is simple, notice how the unconditional is just
sum(beta_i.*Params.massofptypes)

% You can see this is correct, as it is just
Params.beta
% and 
sum([Params.beta.patient,Params.beta.impatient].*Params.massofptypes)
