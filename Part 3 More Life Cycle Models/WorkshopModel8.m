...
%% Setup for Epstein-Zin preferences
...
% EZ preferences are different for positive and negative valued utility functions, by default it would be assumed that 
% the utility is negative, but let's just be explicit and specify this anyway
...
Params.phi=2; % phi>0 'increases' the risk aversion

% 3. Minor adjustment to 'discount factors' and 'return function'.
% We want to distinguish conditional survival probabilities from the regular discount factor. 
% So below the discount factor is now just beta, and we add
vfoptions.survivalprobability='sj';

% Comment: VFI Toolkit only needs to know about Epstein-Zin preferences when solving for Policy (so in vfoptions). Once 
% we have the Policy, the preferences are no longer directly relevant to anything else in the model so we don't need, 
% e.g., to mention them in simoptions.
...
%% ReturnFn
% Discount factors
DiscountFactorParamNames={'beta'}; % We are treating conditional survival probabilities as distinct because of EZ prefs
...
% Divide-and-conquer and grid interpolation layer have not yet been implemented for these alternative preferences
tic;
